...
def calculate_image_dHash(image1):
    # 读取图片并将其转换为灰度图像
    img = cv2.imread(image)
    gray_img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    # 缩放图像尺寸为9x8
    resized_img = cv2.resize(gray_img, (9, 8))
    # 计算像素差值
    diff = resized_img[:, 1:] > resized_img[:, :-1]
    # 将差值转换为D哈希
    dhash = np.packbits(diff.flatten())

    return dhash


def calculate_dhash_similarity(dhash1, dhash2):
    # 计算汉明距离
    hamming_distance = np.count_nonzero(dhash1 != dhash2)

    # 计算相似度
    similarity = 1 - hamming_distance / len(dhash1) * 8
